...
use http_body_util::BodyExt; // For into_data_stream
...
pub async fn start_gateway() {
    let auth_service_url = env::var("AUTH_SERVICE_URL").unwrap_or_else(|_| "http://localhost:8000".to_string());
    let main_service_url = env::var("MAIN_SERVICE_URL").unwrap_or_else(|_| "http://localhost:8002".to_string());
    let realtime_service_ws_url = env::var("REALTIME_SERVICE_WS_URL").unwrap_or_else(|_| "ws://localhost:8001".to_string());
    ...
    let app = Router::new()
        ...
        .route("/api/main", any(proxy_main))
        .route("/api/main/{*path}", any(proxy_main))
...
async fn proxy_auth(State(state): State<AppState>, req: Request<Body>) -> Result<Response<Body>, StatusCode> {
...
async fn proxy_main(State(state): State<AppState>, req: Request<Body>) -> Result<Response<Body>, StatusCode> {
    let path = req.uri().path();
    let path_query = req
        .uri()
        .path_and_query()
...
async fn proxy_request(client: Client, req: Request<Body>, uri: String) -> Result<Response<Body>, StatusCode> {
...
async fn proxy_ws(ws: WebSocketUpgrade, State(state): State<AppState>, req: Request<Body>) -> Response<Body> {
    let path = req.uri().path();
    let path_query = req
        .uri()
        .path_and_query()
