...
**Product Requirements Document.** Dokumen ini menyatukan _apa yang dibangun_,
_untuk siapa_, dan _sejauh mana sudah jadi_. Setiap klaim tentang status
...
Yang benar-benar diproses sistem hari ini: _akses berbayar bertingkat (tipe akun
× durasi) yang, setelah lunas, menaikkan role user menjadi `Member` dan mencatat
Discord username untuk pemberian akses grup_.
...
| F9.17 | Halaman status memakai URL gateway                                                       | ✅     | `status-content.tsx` memakai `NEXT_PUBLIC_API_URL` + `ENDPOINT.MAIN_SERVICE.TRANSACTION_SYNC` |
