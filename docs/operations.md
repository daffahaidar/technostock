...
| Halaman status pembayaran hard-code `http://localhost:8002` + salah prefix                                                                                                                   | Lewat `NEXT_PUBLIC_API_URL` + `ENDPOINT.MAIN_SERVICE.TRANSACTION_SYNC`                              |
...
| `SuperAdmin` tidak selaras antar lapisan                                                                                                                                                     | Diganti `Owner` di DB (migrasi `20260831060000`), Rust, Go, dan frontend; untuk sekarang setara `Admin` |
...
| `http://localhost:3000` / `:8080` tidak bisa dibuka dari browser Windows, padahal container jalan | Relay `localhost` WSL (mode NAT) menerima koneksi lalu memutusnya. Buat `%USERPROFILE%\.wslconfig` berisi `[wsl2]` + `networkingMode=mirrored`, lalu `wsl --shutdown` dan `podman machine start`. Cek cepat: `podman machine ssh curl -s http://127.0.0.1:<port>` — kalau dari dalam VM jalan tapi dari Windows tidak, ini penyebabnya |
